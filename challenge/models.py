...
class Category(models.Model):
    CATEGORY_CHOICES = (('Web', 'Web'), ('Pwn', 'Pwn'), ('Reverse', 'Reverse'),('Crypto', 'Crypto'), ('Misc', 'Misc'), ('Other', 'Other'))
    # category
...
class Challenge(models.Model):
    ...
    CATEGORY_CHOICES = (('Web', 'Web'), ('Pwn', 'Pwn'), ('Reverse', 'Reverse'),('Crypto', 'Crypto'), ('Misc', 'Misc'), ('Other', 'Other'))
    environment_type = models.CharField(choices=ENVIRONMENT_TYPE_CHOICES, default='docker', max_length=20, verbose_name='类型')
    title = models.CharField(max_length=128, verbose_name='题目名称')
    category = models.CharField(choices=CATEGORY_CHOICES,max_length=64, verbose_name='分类')
...
class ChallengeAttachment(models.Model):
    file = models.FileField(upload_to='challenge_file/',
                            verbose_name='文件', help_text='若设置有题目附件请填写此项')
    challenge_one = models.OneToOneField(
        Challenge, on_delete=models.CASCADE, verbose_name='题目', related_name='attachment')
...
class ChallengeDockerConfig(models.Model):
    name = models.CharField(
        max_length=256, verbose_name='镜像名', help_text='若设置题目类型为docker请填写此项')
    port = models.PositiveIntegerField(default=80, verbose_name='内部端口')
    memory_limit = models.PositiveSmallIntegerField(
        default=128, help_text='单位为M', verbose_name='内存限制')
    cpu_limit = models.DecimalField(
        max_digits=3, decimal_places=1, default=0.5, verbose_name='CPU限制')
    challenge_two = models.OneToOneField(
        Challenge, on_delete=models.CASCADE, related_name='docker_config')
